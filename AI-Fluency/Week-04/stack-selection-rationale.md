...
* **Backend needed at launch:** No. Everything ships as a static site.
...
1. **No-Code Builder (Carrd / Framer / Canva)**
   * **How to build & Where to host:** Drag-and-drop visual editors via a browser dashboard and hosted on Carrd or Framer subdomains (Free tiers).
   * **Backend required:** No.
...
2. **Plain HTML, CSS and JavaScript (with AI assistance)**
   * **How to build & Where to host:** Writing semantic HTML and modular CSS/JS files with AI guidance. Hosted on Netlify Drop, GitHub Pages, or Vercel.
   * **Backend required:** No.
...
3. **Next.js + TypeScript + Tailwind CSS (Vercel Framework Stack)**
   * **How to build & Where to host:** Component-driven architecture using React components, TypeScript for type safety and Tailwind CSS for styling. Hosted on Vercel (Free Tier).
   * **Backend required:** No, not for a static portfolio.
...
* **What do I actually maintain if I pick the most powerful option?**
  Real ongoing cost, not zero: build configuration, occasional dependency updates, routing setup and the possibility of build errors when something in the toolchain shifts. Trading for the reusability and structure I get in return.

* **Can I finish in two weeks?**
  Yes. I already have working React/Next.js/TypeScript experience, so I'm applying skills I have rather than learning the stack and building at the same time.

* **Does it show my work the way it needs to be shown?**
  Yes. Code snippets, responsive UI previews and case studies all render cleanly as React components. The no-code option fails this requirement outright since it can't demonstrate engineering skill and plain HTML/CSS can do it but with more manual repetition per page.
...
**Backend at launch:** Not needed. This stays a static site pushed through Vercel.

**Why this choice:** Plain HTML/CSS would honestly be the easier and faster road. But I picked Next.js and Tailwind because it's the stack I'm already actively learning and using, and a portfolio built with it doubles as proof I can work with the tools I claim to know. Pushing updates through GitHub to Vercel also keeps me in a normal developer workflow instead of a one-off publishing process I'll have to relearn later.

**Can I maintain this:** Yes. I already know React basics and have hands-on time with Next.js and Tailwind, so I can read, modify and debug the project structure myself, with AI as a collaborative partner.
...
- *Plain HTML/CSS/JS:* the "simplest" choice and genuinely tempting for how fast and low-maintenance it is, but it would mean hand-managing repeated layout across multiple case study pages and it doesn't showcase the modern tooling I'm actually building experience in.
